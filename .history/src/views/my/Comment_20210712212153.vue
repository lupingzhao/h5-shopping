
<template>
  <div class="comment">
    <!-- 所有评论 -->
    <div class="p-10 t-a-c font-s-16 flex jcsb">
      <div><i class="iconfont icon-zuojiantou" @click="goback"></i></div>
      <div>评价中心</div>
      <div></div>
    </div>
    <div>
      <img src="../../assets/images/evaluate.jpg" alt="" class="img" />
    </div>

    <van-tabs v-model="active" sticky :offset-top="0" @change="change">
      <van-tab :title="title[0].name">
        <!-- 已评价 -->
        <van-list
          v-model="loading"
          :finished="finished"
          finished-text="没有更多了"
          @load="onLoad"
          :immediate-check="false"
        >
          <div v-if="title[0].data.length > 0" class="content">
            <!-- 上滑加载更多数据 -->

            <div
              v-for="(item1, index1) in title[0].data"
              :key="index1"
              class="goodsBox flex a-i-fs"
            >
              <div @click="goDetail('/Details', item1.cid)">
                <img :src="item1.goods[0].image_path" alt="" class="goodImg" />
              </div>
              <div>
                <div>{{ item1.goods[0].name }}</div>

                <div class="p-tb-10 ellipsis">详情： {{ item1.content }}</div>
                <div
                  class="flex share gray"
                  @click.stop="
                    view('/ViewComment', item1.cid, item1.goods[0]._id)
                  "
                >
                  <i class="iconfont icon-pingjia"></i>
                  <div class="pl-10">查看评价</div>
                </div>
              </div>
            </div>
          </div>
          <div v-else class="t-a-c p-10">暂无记录</div>
        </van-list>
      </van-tab>
      <van-tab :title="title[1].name">
        <!-- 未评价 -->
        <van-list
          v-model="loading1"
          @load="onLoad1"
          :immediate-check="false"
          :finished="finished1"
          finished-text="没有更多了"
        >
          <div v-if="title[1].data.length > 0">
            <div
              v-for="(item1, index1) in title[1].data"
              :key="index1"
              class="goodsBox flex a-i-fs"
            >
              <div @click="goDetail('/Details', item1.cid)">
                <img :src="item1.image_path" alt="" class="goodImg" />
              </div>
              <div>
                <div>{{ item1.name }}</div>

                <div
                  class="flex share red"
                  @click="go('/PublishComment', item1)"
                >
                  <i class="iconfont icon-pingjia"></i>
                  <div class="pl-10">评论晒单</div>
                </div>
              </div>
            </div>
          </div>
          <div v-else class="t-a-c p-10">暂无记录</div>
        </van-list>
      </van-tab>
    </van-tabs>
  </div>
</template>

<script>
export default {
  name: "",
  props: {},
  data() {
    return {
      loading: false,
      finished: false,
      loading1: false,
      finished1: false,
      title: [
        {
          name: "已评价",
          data: [],
        },
        {
          name: "待评价",
          data: [],
        },
      ],
      active: 0,
      page: 1,
      total: null,
      total1: null,
    };
  },
  components: {},
  methods: {
    // 返回
    goback() {
      this.$router.back();
    },
    // 已评价
    alreadyEvaluated() {
      this.$api
        .alreadyEvaluated({ page: this.page })
        .then((res) => {
          console.log(res);
          this.total = res.data.count;
          this.title[0].data.concat(res.data.list);
        })
        .catch();
    },
    // 未评价
    tobeEvaluated() {
      this.$api
        .tobeEvaluated({ page: this.page })
        .then((res) => {
          this.total1 = res.data.count;
          this.title[1].data.concat(res.data.list);
        })
        .catch();
    },
    //跳转
    go(val, value) {
      this.$publicJs.go(val, "id", JSON.stringify(value));
    },
    // 查看
    view(path, cid, _id) {
      this.$router.push({
        path: path,
        query: {
          id: cid,
          _id: _id,
        },
      });
    },
    // 去详情
    goDetail(val, value) {
      this.$publicJs.go(val, "id", value);
    },
    // 已评价的加载事件
    onLoad() {
      this.page++;
      console.log(this.page);
      // 异步更新数据
      // setTimeout 仅做示例，真实场景中一般为 ajax 请求
      // 加载状态结束
      this.loading = false;
      this.alreadyEvaluated();
      // 数据全部加载完成
      if (this.title[0].data.length >= this.total) {
        this.finished = true;
      }
    },
    // 未评价加载事件
    onLoad1() {
      this.page++;
      // 异步更新数据
      setTimeout(() => {
        // 加载状态结束
        this.loading1 = false;
        this.tobeEvaluated();
        // 数据全部加载完成
        if (this.title[1].data.length >= this.total1) {
          this.finished1 = true;
        }
      }, 2000);
    },

    //标题切换事件
    change() {
      this.page = 1;
      this.loading = false;
      this.finished = false;
    },
  },
  mounted() {
    this.alreadyEvaluated();
    this.tobeEvaluated();
    // console.log(this.title);
  },
  computed: {},
  watch: {},
};
</script>

<style lang='scss' scoped>
.comment {
  .img {
    width: 100%;
    height: 250px;
  }
  .goodsBox {
    width: 90%;
    margin: auto;
    margin-bottom: 10px;
    margin-top: 10px;
    padding: 10px;
    background-color: #fff;
    border-radius: 10px;
    position: relative;
    height: 100px;

    .share {
      width: 100px;
      text-align: center;

      font-size: 12px;
      padding: 5px 10px;

      border-radius: 10px;
      position: absolute;
      right: 10px;
      bottom: 10px;
    }
    .red {
      color: red;
      border: 1px solid red;
    }
    .gray {
      color: gray;
      border: 1px solid gray;
    }
  }
  .goodImg {
    width: 80px;
  }
}
</style>